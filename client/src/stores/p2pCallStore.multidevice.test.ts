...
vi.mock("../native/nativeP2PCall", () => ({ NativeP2PCall: { discardOrphanedCall: vi.fn(async () => ({ discarded: true })) } }));
...
describe("taking over a call a previous page ran", () => {
  ...
  ])("hangs up in the old page's name when %s", async (_label, outcome) => {
    useP2PCallStore.getState().holdAdoptableCall(candidate);
    useP2PCallStore.getState().resumeCallAfterReconnect();
    sent.length = 0;
  ...
  it("gives the call up even when the page never connects", async () => {
    useP2PCallStore.getState().holdAdoptableCall(candidate);
