...
func initHandlers(svcs *Services, repos *Repositories, limiters *RateLimiters, hub *ws.Hub, cfg *config.Config, encryptionKey []byte, urlSigner services.FileURLSigner) *Handlers {
	...
	h := &Handlers{
		...
		ReadState:         handlers.NewReadStateHandler(svcs.ReadState, limiters.Read),
		DM:                handlers.NewDMHandler(svcs.DM, svcs.DMUpload, svcs.Storage, cfg.Upload.MaxSize, limiters.Message, limiters.Read, urlSigner),
